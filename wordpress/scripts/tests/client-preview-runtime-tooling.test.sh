...
grep -Fq 'ROSA_REFERENCE_URL' "$RUNTIME" || fail 'runtime verifier does not expose optional strict finished-target reference mode'
grep -Fq 'client-preview-secondary-fidelity.test.mjs' "$RUNTIME" || fail 'runtime verifier omits secondary-page finished-target fidelity acceptance'
grep -Fq 'node --check wordpress/scripts/tests/client-preview-secondary-fidelity.test.mjs' "$RUNTIME" || fail 'runtime verifier omits secondary fidelity syntax gate'
